...
def dfs(s, h): # staff / height of tower
    if h >= shelf:
        towers.append(h)
    if s == staff: # len(heights)
        if h >= shelf:
            towers.append(h)
        return
...
for test_case in range(1, int(input())+1):
    staff, shelf = map(int, input().split()) # number of staffs & height of shelf
    heights = list(map(int, input().split())) # each staff's height

    towers = []
    dfs(0, 0)

    tower = min(set(towers)) - shelf


    print('#{} {}'.format(test_case, tower))
